...
func (d *DropDown) handleEvents(gtx layout.Context) {
	if d.IsOpen {
		for i := range d.items {
			index := i
			for d.items[index].clickable.Clicked(gtx) {
	...
	for d.backdrop.Clicked(gtx) {
		d.closeAllDropdown(d.Group)
	}
}

func (d *DropDown) Changed(gtx layout.Context) bool {
	if d.IsOpen {
		for i := range d.items {
			index := i
			for d.items[index].clickable.Clicked(gtx) {
...
func DisplayOneDropdown(gtx layout.Context, dropdowns ...*DropDown) {
	var menus []*DropDown
