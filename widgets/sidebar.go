...
func NewSidebar(theme *chapartheme.Theme) *Sidebar {
	s := &Sidebar{
		...
		Buttons: []*SideBarButton{
			{Icon: widgets.SwapHoriz, Text: "Requests", MarginRight: 17},
			{Icon: widgets.MenuIcon, Text: "Envs", MarginRight: 40},
			{Icon: widgets.WorkspacesIcon, Text: "Workspaces"},
			{Icon: widgets.FileFolderIcon, Text: "Proto files", MarginRight: 10},
...
func (s *Sidebar) makeButtons(theme *chapartheme.Theme) {
	s.flatButtons = make([]*FlatButton, 0)
	for i, b := range s.Buttons {
		s.flatButtons = append(s.flatButtons, &FlatButton{
			Icon:              b.Icon,
			Text:              b.Text,
			IconPosition:      widgets.FlatButtonIconStart,
